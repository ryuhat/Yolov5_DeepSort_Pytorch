...
def parse_opt():
    ...
    parser.add_argument('--yolo-model', type=str, default=WEIGHTS / './best-seg.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--source', type=str, default='./videos/733.mp4', help='file/dir/URL/glob, 0 for webcam')  
    ...
    parser.add_argument('--save', action='store_true', help='save video tracking results', default=True)
    ...
    parser.add_argument('--save-txt', action='store_true', help='save tracking results in a txt file', default=True)
...
def object_detection_and_tracking(video):
    opt = parse_opt()
    opt.source = video
    main(opt)
...
#     opt.source = video
#     main(opt)
#     output_dir = opt.project / opt.name
#     # Get the latest output directory
#     output_dirs = sorted(output_dir.parent.glob("exp*"))
#     latest_output_dir = output_dirs[-1] if output_dirs else None
#     if latest_output_dir:
#         output_video_path = latest_output_dir / (pathlib.Path(video).stem + ".mp4")
#         # Move the output video to a temporary directory
#         with tempfile.TemporaryDirectory() as temp_dir:
#             temp_output_path = shutil.move(str(output_video_path), temp_dir)
...
def object_detection_and_tracking(video):
    ...
    # Get the latest output directory
    output_dirs = sorted(output_dir.parent.glob("exp*"))
    latest_output_dir = max(output_dirs) if output_dirs else None
    if latest_output_dir:
        output_video_path = latest_output_dir / (pathlib.Path(video).name)
        # Move the output video to a temporary directory
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_output_path = shutil.move(str(output_video_path), temp_dir)
            return temp_output_path
...
gr_interface = gr.Interface(fn=object_detection_and_tracking, inputs="video", outputs="video", capture_session=True)
